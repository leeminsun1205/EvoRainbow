...
parser.add_argument('-render', help='Render gym episodes', action='store_true')  # render argument
...
if parameters.render:
    video_dir = os.path.join(parameters.save_foldername, "videos")
    if not os.path.exists(video_dir):
        os.makedirs(video_dir)
    env = gym.make(parameters.env_name)
    env = Monitor(env, video_dir, force=True, video_callable=lambda ep: True)
else:
    env = gym.make(parameters.env_name)
            ...
            if previous_info is not None:
                parameters.wandb.log(
                    {'Pop_mean': current_info[0], 'Pop_max':current_info[1], 'Pop_min': current_info[2],
                     'Pop_improve': current_info[0] - previous_info[0], 
                     'Pop_max_update':current_info[1]- previous_info[1],
                     'elite': elite_rate, 'selected': win_rate, 'discarded': dis_rate,
                     'best_reward': best_reward, 'add_rewards': add_rewards,
                     'pvn_loss': pvn_loss, 'keep_c_loss': keep_c_loss, 'l1_before_after': l1_before_after,
                     'mu_score': mu_score, 'pre_loss': pre_loss, 'num_frames': agent.num_frames, 
                     'num_games': agent.num_games, 'erl_score': erl_score, 'RL_reward': RL_reward,
