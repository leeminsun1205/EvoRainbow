...
from EvoRainbow_core.mod_utils import is_lnorm_key
import numpy as np
...
class Champion(object):
    ...
    def get_action(self,state):
        ...
        action = self.actor.select_action_from_z(s_z)
        return action
...
class TD3(object):
    ...
    def train(self, SAME, champion ,evo_times,all_fitness, all_gen , on_policy_states, on_policy_params, on_policy_discount_rewards,on_policy_actions,replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.005, policy_noise=0.2,
        ...
        div_num = int(iterations / 5)
        #print(SAME)
        for it in range(iterations):
            if it% div_num == 0 and it !=0 and self.args.Soft_Update == 2 and not SAME:
                x, _, _, _, _, _, _ = replay_buffer.sample(512)
                stateIn = torch.FloatTensor(x).to(self.device)
                s_z = self.state_embedding.forward(stateIn)
                rl_action = self.actor.select_action_from_z(s_z)
                champion_action = champion.get_action(stateIn)
                post_distance = ((rl_action - champion_action) ** 2).mean().item()

                #print(SAME," Current distance ", post_distance, self.guide_alpha)
                if post_distance > 0.05 * 1.5:
                    self.guide_alpha *= 2
                    self.guide_alpha = 1024 if self.guide_alpha > 1024 else self.guide_alpha
                elif post_distance < 0.05 / 1.5:
                    self.guide_alpha /= 2
            ...
            if it % policy_freq == 0:
                ...
                s_z= self.state_embedding.forward(state)
                rl_action = self.actor.select_action_from_z(s_z)
                actor_loss = -self.critic.Q1(state, rl_action).mean()

                if not SAME  and self.args.Soft_Update == 2:
                    champion_action = champion.get_action(state).detach()
                    distance = ((champion_action - rl_action) ** 2).mean()
                    #print(distance.item())
                    actor_loss = actor_loss + self.guide_alpha * distance
